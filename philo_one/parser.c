...
/*   Updated: 2021/05/20 11:50:26 by hveiled          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mallocate(t_all *all, int i)
{
	...
	all->mtx = (pthread_mutex_t **)malloc(sizeof(pthread_mutex_t *)
			* all->arg.nmbr_of_philosophers);
	i = -1;
	while (++i < all->arg.nmbr_of_philosophers)
	{
		all->mtx[i] = (pthread_mutex_t *)malloc(sizeof(pthread_mutex_t));
		if (!all->mtx[i])
			return (0);
	}
	if (!all->philo || !all->thread || !all->mtx)
		return (0);
	return (1);
}
...
int	pars_line(t_all *all, int ac, char *av[], int i)
{
	if (error_check(ac, av))
		return (0);
	all->waiter_mtx = malloc(sizeof(*(all->waiter_mtx)));
